...
# ユーザー名でユーザーを取得(ユーザー登録時に登録済みかどうかを確認するため)
...
# 会議室名で会議室を取得(会議室登録時に登録済みかどうかを確認するため)
def get_room_by_room_name(db: Session, room_name: str):
    return db.query(models.Room).filter(models.Room.room_name == room_name).first()
